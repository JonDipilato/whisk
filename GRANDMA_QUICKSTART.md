...
**Important:** Always pass `--output-dir` when running `run_story.py` manually. Without it, the pipeline uses the default `output/` folder which has your Luna/Kai images and will skip scene generation.
...
Grandma episodes have their own counter at `data/episode_counter_grandma.json` (separate from Luna/Kai).
